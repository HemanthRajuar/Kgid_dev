...
namespace KGID.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public void Return(string encdata, string dept_code, string urlId, string connect)
        {
            ...
            Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success MBPay :" + Convert.ToString(Session["KIIReturn"]));

            long result = 1;
            Logger.LogMessage(TracingLevel.INFO, "objPaymentDetails :" + "MBPay");

            if (Session["KIIReturn"].ToString() == "MBPay")
            {
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success Return to   MotorBranchPay :" + Convert.ToString(Session["KIIReturn"]));
                ...
                Logger.LogMessage(TracingLevel.INFO, "aftter GetTranDataFromString for Motorbranch Details log:" + resdecData);

                //objPaymentDetails.cs_status = 2;
                //objPaymentDetails.cs_amount = objPaymentDetails.cs_amount;
                //objPaymentDetails.cd_challan_ref_no = deptRefNum;
                //objPaymentDetails.cs_status = 2;
                //objPaymentDetails.cs_transsactionMB_date = DateTime.UtcNow;
                //objPaymentDetails.cs_transaction_ref_no = objPaymentDetails.cs_transaction_ref_no;
                //objPaymentDetails.EmpID =Convert.ToInt64(Session["UID"]);
                try
                {
                    Logger.LogMessage(TracingLevel.INFO, "cs_transaction_ref_no" + objPaymentDetails.cs_transaction_ref_no);
                    Logger.LogMessage(TracingLevel.INFO, "cd_challan_ref_no :" + objPaymentDetails.cd_challan_ref_no);
                ...
                {
                    Logger.LogMessage(TracingLevel.INFO, "MotorBranchPay Error Saving Details :" + ex.Message.ToString());
                }
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success Return to   MotorBranchPay :" + result);
            ...
            else
            {
                ...
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session fail to  and redirect to MotorBranchPay :" + resdecData);

                result = _INBApplicationbll.SaveNBChallanStatusDll(objPaymentDetails);
            }
            ...
            else if (Session["KIIReturn"].ToString() == "MBPay")   //ICT Changes 10-Nov-2021
            {
                Logger.LogMessage(TracingLevel.INFO, "MBPAY");
                Logger.LogMessage(TracingLevel.INFO, "UID Session:" + Session["UID"].ToString());
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session fail to  and redirect to MotorBranch controller :" );

                Response.Redirect("/mi-dpt-opsoa");
            }
            if (Session["KIIReturn"].ToString() == "GisPay")
            {
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success Return to   Group Insurace :" + Convert.ToString(Session["KIIReturn"]));

                VM_PaymentDetails objPaymentDetails = GetTranDataFromString(resdecData);

                Logger.LogMessage(TracingLevel.INFO, "objPaymentDetails :" + objPaymentDetails);
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session :" + Convert.ToInt64(Session["UID"]));
                Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success :" + Convert.ToInt64(Session["UID"]));
        ...
        public VM_PaymentDetails GetTranDataFromString(string resdecData)
        {
            ...
            if (Session["KIIReturn"].ToString() == "MBPay")
            {
                foreach (var a in dataArr)
                {
                    ...
                    if (a.Split('=')[0] == "trsn_timestamp")
                        obj.cs_transsaction_date = (a.Split('=') != null) ? a.Split('=')[1].ToString() : "";
                    Logger.LogMessage(TracingLevel.INFO, "GetTranDataFromString for Motorbranch Details:" + a.Split('=')[1].ToString());
                }
            }
        ...
        public ActionResult KIIDoubleVerification(string ReqChallanRefNo)
        {
            ...
            string URL = "https://preprodkhajane2.karnataka.gov.in/KhajaneWs/rct/rrpys/secbc/RctReceivePaymentStatusService?wsdl";
    ...
    + "</soapenv:Envelope>";
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    Logger.LogMessage(TracingLevel.INFO, "Before responseStream");
                    Stream responseStream = response.GetResponseStream();
                    responseStr = new StreamReader(responseStream).ReadToEnd();
                    Logger.LogMessage(TracingLevel.INFO, "responseStr :" + responseStr);
                    responseStr = (responseStr.Replace("\"", "'"));
                    XElement xmlDocumentWithoutNs = RemoveAllNamespaces(XElement.Parse(responseStr));
                    // Logger.LogMessage(TracingLevel.INFO, "After ReadToEnd MB" + xmlDocumentWithoutNs);
                   ...
                   // jsonText = JsonConvert.SerializeXmlNode(doc1.ChildNodes[0].ChildNodes[0].ChildNodes[0], Newtonsoft.Json.Formatting.None, true);
                    ViewBag.Response = responseStr;
                    Logger.LogMessage(TracingLevel.INFO, "RCT34 - doc.InnerText" + doc1.InnerText);
