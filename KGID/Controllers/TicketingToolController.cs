...
namespace KGID.Controllers
{
    public class TicketingToolController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UploadExcel1(ExcelSheetUploadModel model)
        {
            try
            {

                string result = _TicketingToolbll.FileUploadbll(model._ExcelSheetBind._excelFile, model._ExcelSheetBind._uploadType, model._ExcelSheetBind._masterTableId);
            ...
            { // PageName,Message,Source,TargetSite-Name
                //_masterbll.ErrorLogbll(Path.GetFileName(Request.Path), ex.Message, ex.StackTrace, ex.TargetSite.Name);
                //return View();
                throw;
            }
            finally
        ...
        [Route("vehicle-mapping-to-ddo")]
        public ActionResult VehicalMappingToDDO()
        {
        ...
        #region[Vehical Mapping To DDO]
        public JsonResult GetVehicalDetailsUsingVehicalNumber(string VehicalNumber)
        {
            var result = _TicketingToolbll.GetVehicalDetailsUsingVehicalNumber_bll(VehicalNumber);
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetDdoCode(string ddocode)
        {
            DataSet result = _TicketingToolbll.GetDdoCode_bll(ddocode);
            return Json(result.GetXml(), JsonRequestBehavior.AllowGet);
        }

        public JsonResult SaveDDoMapping(List<VehicalMappingToDDO> VehicalMappingToDDO)
        {
            string data = this._TicketingToolbll.SaveDDoMapping_bll(VehicalMappingToDDO);
