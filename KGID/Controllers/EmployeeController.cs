...
namespace KGID.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        [Route("kgid-ExistingInsapp")]
        public ActionResult GetExistEmployeeInstructions()
        { 
            long EmployeeID = 0;
            //  Session["FirstKGIDNo"]
            //var empage = 18;
            ...
            {
                ViewBag.Instructions = "1";
                string Kgidno = Session["FirstKGIDNo"].ToString();
                ViewBag.Kgidno = Kgidno;
                return View("_ExistingEmployeeInstrution");
            }
            else if (CheckEmpAgeStatus == "-1")
            {
                ViewBag.Instructions = "-1";
        ...
        ///     //tHIS is for Basic Details view for Existing employee Application form
        [Route("kgid-existing-view-app")]
        [CustomAuthorize("Employee")]
        public ActionResult ExistingApplicationForm()
        {
        ...
        //tHIS is for Basic Details view for Existing employee
        public ActionResult ExistingBasicDetailsToView()
        {
            ...
            {
                if (_BasicData.dr_id == 1 && (_BasicData.divorced_upload_doc_path != null &&  _BasicData.divorced_upload_doc_path !=""))
                {
                    _BasicData.dr_id = 1;
                }
                else
            ...
            return this.PartialView("_ExistingEmployeeBasicDetails", _BasicData);
        }
        public ActionResult ExistingKGIDDetailsToView()
        {
            var totalamt = 0;
            var amt = 0;
            var amt1 = 0;
            var premiumamts = 0;
            var premiumpayamts = 0;
            VM_PolicyDetails _KGIDData = _INBApplicationbll.KGIDDetailsBll(Convert.ToInt64(Session["UID"]));
         //   _KGIDData.KGIDPolicyList = _INBApplicationbll.KGIDDetailsBll(Convert.ToInt64(Session["UID"]));
            var b = _INBApplicationbll.KGIDDetailsBll(Convert.ToInt64(Session["UID"]));
            _KGIDData.listDashboardData = _INBApplicationbll.GetDetailsBasedOnKGIDNo(Convert.ToInt64(Session["FirstKGIDNo"]));
            long r = Convert.ToInt64(Session["FirstKGIDNo"]);
           // var _empDataList = new object();
          var  _empDataList= _INBApplicationbll.GetDetailsBasedOnKGIDNo(Convert.ToInt64(Session["FirstKGIDNo"]));
            decimal preamt = default(decimal);
            foreach(var c in _empDataList)
            {
                preamt = preamt + c.PremiumAmt;
                //amt1 = Convert.ToInt16(c.PremiumAmt);
                //  amt = Convert.ToInt16(c.PremiumAmt) + amt;
                //premiumamts = amt;
            }

            premiumpayamts = Convert.ToInt16(_KGIDData.premium_Amount) - Convert.ToInt16(preamt);
            _KGIDData.premium_Amount_to_Pay = premiumpayamts;
            _KGIDData.premium_Amount = premiumpayamts;
            //_KGIDData.p_premium = premiumpayamts;
            //_KGIDData.p_premium = premiumpayamts + amt;
            if (_KGIDData.KGIDPolicyList != null)
            {
                double preamt1 = default(double);
                foreach (var c in _KGIDData.KGIDPolicyList)
                {
                    _KGIDData.premium_Amount_to_Pay = premiumpayamts; Convert.ToDecimal(c.p_premium);
                    _KGIDData.premium_Amount = premiumpayamts;//Convert.ToDecimal(c.p_premium);
                    //preamt1 = preamt1 + Convert.ToDouble(c.p_premium) + Convert.ToDouble(preamt);
                    preamt1 = preamt1 + Convert.ToDouble(c.p_premium) ;
                    c.p_premium  = preamt1;
                }
            }
            else {
               // _KGIDData.p_premium =Convert.ToDouble(_KGIDData.premium_Amount) + Convert.ToDouble(preamt);
            ...
            return this.PartialView("_ExistingKGIDDetails", _KGIDData);
        }

        public ActionResult ExistingPersonalDetailsToView()
        {
            ...
            return this.PartialView("_ExistingPersonalDetails", _PersonalData);
        }
        public ActionResult ExistingDeclarationDetailsToView()
        {
            return this.PartialView("_ExistingDeclarationDetails");
        }
        public ActionResult ExistingFamilyDetailsToView()
        {
            ...
            if (_FamilyData.FamilyDetails == null)
            {
                foreach (var s in _FamilyData.FamilyDetails)
                {
                    s.Relation = "Father";
                    s.Relation = "Mother";
                }
            ...
            return this.PartialView("_ExistingFamilyDetails", _FamilyData);
        }
        ...
        //    return result;
        //}
        //public JsonResult InsertBasicDetails(VM_BasicDetails objBasicDetails)
        //{
        //    string nom = "";
        //    bool isSuccess = false;
        //    string message = string.Empty;
        //    string result = "";
        //      result = _INBApplicationbll.SaveNBBasicBll(objBasicDetails);
        //    var nomineDet = (from n in _db.tbl_employee_nominee_details

        //                     where n.end_emp_id == objBasicDetails.employee_id
        //                     select n).FirstOrDefault();

        //    if (nomineDet != null)
        //    {
        //        isSuccess = false;
        //        message = "Please Remove the nominee Details";
        //        ViewBag.mess = "Please Remove the nominee Details";
        //        result = "1";
        //    }
        //    else
        //    {
        ...
        //        {
        //            if (duplicate.Contains("Pan number") || duplicate.Contains("Email id"))
        //            {
        ...
        public JsonResult InsertBasicDetails(VM_BasicDetails objBasicDetails)
        {
            bool isSuccess = false;
            string message = string.Empty;
            string result = _INBApplicationbll.SaveNBBasicBll(objBasicDetails);
            if (result != "2" && result != "1")
            {
                string[] duplicate = result.Split(',');
        ...
        [Route("kgid-update-ApplicationMapping")]
        public ActionResult UpdateApplicationMapping()
        {
            return View();
        }

        [Route("kgid-DispalyDetails-updateApplicationMapping")]
        public ActionResult DisplayDetailsForUpdateApplicationMapping(string ApplicationRefnum)
        {
            List<VM_ApplicationDetail> result = _INBApplicationbll.GetApplicationMappingDetails(ApplicationRefnum);
            ViewBag.ApplicationMappingDeatils = result;
            return PartialView("DisplayUpdateAppMappingdeatils", result);
        }

        [Route("kgid-DispalyDetails-EmployeeDetails")]
        public ActionResult DisplayDetailsEmployeeDetails(long kgidNo)
        {
            List<VM_ApplicationDetail> result = _INBApplicationbll.GetemployeeDetailsBasedOfKGIDNo(kgidNo);
            ViewBag.EmployeeDetails = result;
            return PartialView("DisplayEmployeeDetails", result);
        }

        [Route("Update-AssignedTo")]
        public async Task<ActionResult> UpdateAssignedToWFD(long EmployeeId, long kawt_workflow_id, int kawt_application_status)
        {
            return Json(await _INBApplicationbll.UpdateAssignedTo(EmployeeId, kawt_workflow_id, kawt_application_status) ? 1 : 0);
        }
        #endregion

        //1 jan 2022 JSE
        [Route("ProposerTrackDetails-1")]
        public ActionResult Track_Proposal_NumberDetails()
        {
            return View();
        }
        //1 jan 2022 JSE        
        public JsonResult getProposerTrackDetails1(VM_ApplicationDetail employeeDetails)
        {
            string applicationNo = employeeDetails.ApplicationNumber;
            VM_ApplicationDetail obj = new VM_ApplicationDetail();
            obj.listTrackDetails = _INBApplicationbll.getProposerTrackDetailsBll(applicationNo);
