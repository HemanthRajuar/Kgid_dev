...
namespace KGID.Controllers
{
    ...
    public class MotorInsuranceController : Controller
    {
        ...
        public JsonResult InsertMIAppnReferenceno(VM_MotorInsuranceProposerDetails objMIPDetails)
        {
            ...
            if (objMIPDetails.mipd_pagetype == "Renewal" || objMIPDetails.mipd_pagetype == "EditRenewal")
            {
        ...
        public JsonResult InsertIDVDetails(VM_MotorInsuranceIDVDetails objPersonalDetails)
        {
            ...
            return Json(new { IsSuccess = isSuccess, Message = message , pagetype = pagetype }, JsonRequestBehavior.AllowGet);
        }
        //[AcceptVerbs(HttpVerbs.Get | HttpVerbs.Post)]
        ...
        public JsonResult PayMIChallanDetailsKII(string RefNos, string ChallanAmount, string Type)
        {
            ...
            if (ChallanAmount == ChalnAmt && AppRefNo == RefNos)
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    //UAT
                    client.BaseAddress = new Uri("http://49.206.243.84:5050/kgid/v1/getSignature");
                    //client.BaseAddress = new Uri("http://10.96.158.108:5050/SignBinaryData/");
                    //prod
                    // client.BaseAddress = new Uri("http://172.31.39.6:5050/SignBinaryData/");
                    object reqdata = new
                    {
                    ...
                    Logger.LogMessage(TracingLevel.INFO, "After signbinarydata SignedresultContent" + SignedresultContent);
                ...
                try
                {
                    if (SignedresultContent != null)
                    {
                        ...
                        Logger.LogMessage(TracingLevel.INFO, "after GetKIISignDetails data" + data);
        ...
        public string GetKIISignDetails(string signeddata, string xmldata)
        {
            try
            {
                //string URL = "https://preprodk2.karnataka.gov.in/wps/portal/Home/DepartmentPayment/?uri=receiptsample:com.tcs.departmentpage:departmentportlet";

                //string URL =          "https://preprodk2.karnataka.gov.in/wps/portal/Home/DepartmentPayment/?uri=receiptsample:com.tcs.departmentpage:departmentportlet&encdata=S58YWfj6F+hnpSWxjSm9xLXEZTH+j55VtJ5b+SfWe+gZ99BeXmZfyoYyeknGh+uEyrgNscmdZXCZAPbeRDZDQgg9EANCEV7RPBzj2ElNAM5L1esQyhEPK9L1MiEMPkOcg8RQr5pCfUfYEhS9Fq2Ka6IEkOB9DBXO6z8L5zW/5fU=&dept_code=12C
                ...
                Logger.LogMessage(TracingLevel.INFO, xml2);
        ...
        public ActionResult MotorInsuranceRenewalApplication(string PageType = null, long refNo = 0, string category = "", long RenewalRefNo = 0)
        {
            ...
            Session.Remove("RID");
            Session["RID"] = refNo;
            obj.RenewalRefNo = Convert.ToInt64(refNo);
            obj.previousRefNo = RenewalRefNo;
        ...
        public ActionResult RenewalIDVDetailsToView(string PageType, long refNo, long RenewalRefNo)
        {
            //RenewalRefNo = Convert.ToInt64(Session["RID"]);
            Session["RID"] = RenewalRefNo;
            VM_MotorInsuranceIDVDetails _IDVData = new VM_MotorInsuranceIDVDetails();
            _IDVData = _IMotorInsuranceVehicleDetailsBll.IDVRenewalDetailsBll(Convert.ToInt64(Session["UID"]), refNo, RenewalRefNo);
        ...
        public JsonResult GetDateOfRisk(string ReferanceNo)
        {
            var result = _IMotorInsuranceVehicleDetailsBll.GetDateOfRisk_bll(ReferanceNo);
        ...
        public ActionResult RenewalDeclarationToView(string PageType, long refNo, long RenewalRefNo)
        {
            ...
            _VehicleDetails.mivd_pagetype = PageType;
            _VehicleDetails.mi_referenceno = refNo;
        ...
        public ActionResult MotorInsuranceListOfRenewalApplications()
        {
            ...
            if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
            {
                ...
                ApplicationDetails.Category = Convert.ToInt32(Session["SelectedCategory"]);
            }
            else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
            {
                ...
                ApplicationDetails.Category = Convert.ToInt32(Session["Categories"]);
            }
            else
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.getMIRenewalApplicationList(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"]));
        ...
        #region Vehicle Transfer
        //Vehicle Transfer
        public ActionResult VehicleTransfer()
        {
            return View();
        }
        public JsonResult MICheckValidMotorVehicleNo(string VehicleNo)
        {
            string empid = "";
            string appid = "";
            string isSuccess = "";

            string result = _IMotorInsuranceVehicleDetailsBll.CheckTransferVehicleNo(VehicleNo);
            if (result.Length > 1)
            {
                string[] value = result.Split(',');
                empid = value[1];
        ...
        public JsonResult VehicleDetailsForTransfer(string VehicleNo)
        {
            VM_MotorInsuranceVehicleTransfer obj = new VM_MotorInsuranceVehicleTransfer();
            obj = _IMotorInsuranceVehicleDetailsBll.VehicleDetailsForTransfer(VehicleNo);
            return Json(obj, JsonRequestBehavior.AllowGet);
            //return this.PartialView("_MIVehicleDetailsForCancellation", obj);
        }
        #endregion

        //// ICT Source 18-09-2021  -- old
        ...
        //    string billsector = string.Empty;
        //    string SignedresultContent = string.Empty;
        ...
        //            string resFile = TFCNewPolicyPremiumDATA(Convert.ToInt64(amount), deptRefNum, DDOCODE, SANORDERNO, SANDATE, voucherno, hoacode,billsector);
        ...
        public JsonResult PayMIChallanDetails(string RefNos, string ChallanAmount, string DDOCODE, string Type)
        {
            ...
            //if (ChallanAmount == ChallanAmount && RefNos == RefNos)
            if (1 == 1)
            {
                Session["KIIReturn"] = "MBPay";
                string[] subs = RefNos.Split('$');
                ...
                string resFile = TFCNewPolicyPremiumDATA(Convert.ToInt64(amount), deptRefNum, DDOCODE, SANORDERNO, SANDATE, voucherno, hoacode, billsector);
                String vDataContent = System.IO.File.ReadAllText(resFile);
                //string vApiUri = @"http://49.206.243.84:6060/rest-service/kgid/SignXml";
                //string vApiUri = @"http://10.96.158.108:6060/rest-service/kgid/SignXml";
                string vApiUri = System.Configuration.ConfigurationManager.AppSettings["SignXmlURI"];
                StringContent data = new StringContent(vDataContent, Encoding.UTF8, "text/xml");
        ...
        // ICT CHANGE
        public string TFCNewPolicyPremiumDATA(long ChallanAmount, string Refno, string DDOCODE, string SANORDERNO, string SANDATE, string voucherno, string hoacode, string billsector)
        {
            ...
            try
            {
                ...
                using (System.IO.StreamWriter strmwrtr = fi.CreateText())
                {
                    ...
                    strmwrtr.WriteLine("<blSector>" + billsector + "</blSector>");
        ...
        //[Route("mi-dpt-r-psoa")]
        //public ActionResult MotorInsuranceRenewalPaymentStatusOfApplication()
        //{
        //    VM_MotorInsurancePaymentStatus PaymentStatusDetails = null;
        //    if (Convert.ToInt32(Session["SelectedCategory"]) == 2)
        //    {
        ...
        public ActionResult VerifyMIChallanDetails(string ID)
        {
            ...
            if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]));
            }
            else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"]));
            }
            else if (Session["MBVALUE"] != null && Convert.ToBoolean(Session["MBVALUE"]) == true)
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"]));
        ...
        [Route("mi-viewchlrenew-det/{ID}")]
        public ActionResult VerifyMIRenewChallanDetails(string ID)
        {
            VM_DDOVerificationDetailsMI ApplicationDetails = null;

            ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetRenewalChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]));
            return View(ApplicationDetails);
        }
        ...
        public ActionResult PrintMIChallan(string ID)
        {
            ...
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"]));
                ApplicationDetails.DDOCODE = Session["Categories"].ToString();
            }
        ...
        // ict 30-10-2021
        [Route("mi-viewchlotc-det/{ID}")]
        public ActionResult VerifyMIOTCChallanDetails(string ID)
        {
            VM_DDOVerificationDetailsMI ApplicationDetails = null;
            if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetotcChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]));
            }
            else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetotcChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"]));
            }
            else if (Session["MBVALUE"] != null && Convert.ToBoolean(Session["MBVALUE"]) == true)
            {
                ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetotcChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]),Convert.ToInt32(Session["Categories"]));
        ...
        public JsonResult InsertMIOTCChallanDetails(string RefNos, string ChallanAmount, string sanno, DateTime sandate, string voucherno, string hoa, string ddocode, string Type)
        {
            bool isSuccess = false;
            string message = string.Empty;
            string result = string.Empty;
            ...
            if (ChallanAmount == ChalnAmt && AppRefNo == RefNos)
            {
                ...
                if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertotcChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
                }
                else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertotcChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
                ...
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertotcChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["Categories"].ToString()), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
                }
                else
                {
                    //##ICT##
        ...
        public JsonResult PayMIOTCChallanDetails(string RefNos, string ChallanAmount, string DDOCODE, string OTCFIELD, string Type)
        {
            ...
            string DEPTPRPSID = string.Empty;
            string SUBPRPSNAME = string.Empty;
            string RMTRNAME = string.Empty;
            string TRSRYCODE = string.Empty;

            Logger.LogMessage(TracingLevel.INFO, "Inside PayMIOTCChallanDetails");
            ...
            if (1 == 1)
            {
                ...
                Logger.LogMessage(TracingLevel.INFO, "Before TFCNewPolicyPremiumOTC Data File Generation *********");

                string resFile = TFCNewPolicyPremiumOTC(Convert.ToInt64(amount), deptRefNum, DDOCODE, SANORDERNO, SANDATE, voucherno, hoacode, DEPTCODE, DEPTPRPSID, SUBPRPSNAME, RMTRNAME, TRSRYCODE);
                Logger.LogMessage(TracingLevel.INFO, "Data File generated *********");
                Logger.LogMessage(TracingLevel.INFO, resFile);
                byte[] AsBytes = System.IO.File.ReadAllBytes(resFile);

                String vDataContent = System.IO.File.ReadAllText(resFile);
                // for uat
                string vApiUri = System.Configuration.ConfigurationManager.AppSettings["SignXmlURIOTC"];
                // for Produu
                //string vApiUri = System.Configuration.ConfigurationManager.AppSettings["SignXmlURIOTC"];
                StringContent vdata = new StringContent(vDataContent, Encoding.UTF8, "text/xml");

                Logger.LogMessage(TracingLevel.INFO, "Before API Call");
                Logger.LogMessage(TracingLevel.INFO, vApiUri);

                using (var client = new HttpClient())
                {
                    var response = client.PostAsync(vApiUri, vdata).Result;

                    if (response.IsSuccessStatusCode)
                    {

                        SignedresultContent = response.Content.ReadAsStringAsync().Result;
                        Logger.LogMessage(TracingLevel.INFO, SignedresultContent);
                ...
                Logger.LogMessage(TracingLevel.INFO, "MB OTC API Call ended");
                try
                {
                    if (SignedresultContent != null)
                    {
                        Logger.LogMessage(TracingLevel.INFO, "MB OTC after Sign detail");

                        string data = GetKIISignDetails(SignedresultContent, Encoding.UTF8.GetString(AsBytes));

                        Logger.LogMessage(TracingLevel.INFO, "Data :" + data);
                        Logger.LogMessage(TracingLevel.INFO, "MB OTC after GetKIISignDetails");
                        ...
                        if (signedDataResponseK2.statusCode == "KII-RCTER-00" && signedDataResponseK2.statusDescription == "Success")
                        {
                            ...
                            long result1 = 1;
                            string Requestresult = string.Empty;
                            Requestresult = _IMotorInsuranceVehicleDetailsBll.UpdateRCT0335AckBll(deptRefNum, "success", signedDataResponseK2.statusCode + "$" + signedDataResponseK2.statusDescription);
                            string HashChechsumMD5 = "dept_ref_no=" + deptRefNum + "|txn_date=" + transactiondate + "|amount=" + amount + "|dept_pwd=1234";
                            string AfterHashChechsumMD5 = GetMD5Checksum(HashChechsumMD5);
                            string BeforeEncryptedStringData = "dept_ref_no=" + deptRefNum + "|txn_date=" + transactiondate + "|amount=" + amount + "|dept_pwd=1234|checkSum=" + AfterHashChechsumMD5 + "";
                            string EncryptedStringData = SymetricEncrypt(BeforeEncryptedStringData, "EdZUiBM0d8C46PEZ2Yn9Gg==");
                            long empid = Convert.ToInt64(Session["UID"]);
                            string SymmetricDecryptData = SymmetricDecrypt(EncryptedStringData, "EdZUiBM0d8C46PEZ2Yn9Gg==");
                            Logger.LogMessage(TracingLevel.INFO,  "Data :" + SymmetricDecryptData);

                            VM_PaymentDetails objPaymentDetails = GetTranDataFromString(SymmetricDecryptData);
                            objPaymentDetails.cs_amount = Convert.ToInt32(ChallanAmount);
                            objPaymentDetails.cd_challan_ref_no = deptRefNum;
                            objPaymentDetails.cs_status = 1;
                            objPaymentDetails.cs_transsaction_date = Convert.ToString(DateTime.UtcNow);
                            objPaymentDetails.cs_transaction_ref_no = objPaymentDetails.cs_transaction_ref_no;
                            objPaymentDetails.EmpID = empid;
                            result1 = _IMotorInsuranceVehicleDetailsBll.SaveMBChallanStatusDll(objPaymentDetails);
                            Logger.LogMessage(TracingLevel.INFO, "Before K2 Hit");
                            string KIIurl = "https://preprodk2.karnataka.gov.in/wps/portal/Home/DepartmentPayment/?uri=receiptsample:com.tcs.departmentpage:departmentportlet";
                            string redirect_url = "" + KIIurl + "" + "&encdata=" + EncryptedStringData + "&dept_code=12C";
                            Logger.LogMessage(TracingLevel.INFO, "After K2 Hit");
                            result = redirect_url;
                            isSuccess = true;
                            message = "Redirecting to KII Portal";
                            Logger.LogMessage(TracingLevel.INFO, " Before Redirect");
                        ...
                        else
                        {
                            ...
                            long result1;
                            string Requestresult = string.Empty;
                            Requestresult = _IMotorInsuranceVehicleDetailsBll.UpdateRCT0335AckBll(deptRefNum, "fail", signedDataResponseK2.statusCode + "$" + signedDataResponseK2.statusDescription);
                            VM_PaymentDetails objPaymentDetails = new VM_PaymentDetails();
                            objPaymentDetails.cs_amount = Convert.ToInt32(ChallanAmount);
                            objPaymentDetails.cd_challan_ref_no = deptRefNum;
                            objPaymentDetails.cs_status = 1;
                            objPaymentDetails.cs_transsaction_date = Convert.ToString(DateTime.UtcNow);
                            objPaymentDetails.cs_transaction_ref_no = deptRefNum;
                            objPaymentDetails.EmpID = empid;
                            result1 = _IMotorInsuranceVehicleDetailsBll.SaveMBChallanStatusDll(objPaymentDetails);

                            isSuccess = false;
                            message = signedDataResponseK2.statusDescription;
                            return Json(new { IsSuccess = isSuccess, Message = message, Result = result }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                ...
                {
                    isSuccess = false;
                    message = "Payment fail" + ex.Message.ToString();
                    Logger.LogMessage(TracingLevel.INFO, ex.Message.ToString());
                    return Json(new { IsSuccess = isSuccess, Message = message, Result = result }, JsonRequestBehavior.AllowGet);
                    //Log.Error("Error Level", ex);
                }
                //return Json(new { IsSuccess = isSuccess, Message = message, Result = result }, JsonRequestBehavior.AllowGet);
            ...
            else
            {
                ...
                //myremotepostfalse.Url = "https://49.206.243.82:82/kgid-app?pay=false";
        ...
        /// <summary>
        /// This method is no being used should take conclusion from Kunal and team
        /// </summary>
        ...
        public VM_PaymentDetails GetTranDataFromString(string resdecData)
        {
            ...
            foreach (var a in dataArr)
            {
                ...
                if (a.Split('=')[0] == "txn_date")
                    obj.cs_transsaction_date = (a.Split('=') != null) ? DateTime.UtcNow.ToString() : "";
            ...
            Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session If Success Mb PAy :" + Convert.ToInt64(Session["UID"]));

            Logger.LogMessage(TracingLevel.INFO, "KII_Home_Return_UID_Session :" + obj.EmpID);
            return obj;
        }
        public string GetKIISignDetailsOTC(string signeddata, string xmldata)
        {
            try
            {
                string URL = "https://khajane2.karnataka.gov.in/KhajaneWs/rct/rrpys/secbc/RctReceivePaymentStatusService?wsdl";
               Logger.LogMessage(TracingLevel.INFO, "Entering into GetKIISignDetailsOTC() with signeddata= " + signeddata +
                    " and xmldata= " + xmldata + " and URL= "+ URL);
                string xml2 = @"<?xml version='1.0' encoding='UTF-8'?>"
                      + "<ser:envelopedDataReq"
                      + "\n"
                      + "xmlns:head='http://header.ei.integration.ifms.gov.in/'"
                      + "xmlns:ser='http://service.receivevalidatechallan.dept.rct.integration.ifms.gov.in/'"
                ...
                request.ContentType = "text/xml";
                request.ContentLength = bytes.Length;
                Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). XML created is " + xml2 + " . And bytes length is "
                    + bytes.Length);
                request.Method = "POST";
                Stream requestStream = request.GetRequestStream();
                requestStream.Write(bytes, 0, bytes.Length);
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). Success response received from URL" + URL);
                    Stream responseStream = response.GetResponseStream();
                    responseStr = new StreamReader(responseStream).ReadToEnd();
                    XElement xmlDocumentWithoutNs = RemoveAllNamespaces(XElement.Parse(responseStr));
                    XDocument xDoc = XDocument.Load(new System.IO.StringReader(responseStr));
                    var unwrappedResponse = xDoc.Descendants((XNamespace)"http://schemas.xmlsoap.org/soap/envelope/" + "Body").First().FirstNode;
                    XmlDocument doc1 = new XmlDocument();
                    //Load XML unwrappedResponse
                    Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). unwrappedResponse: " + unwrappedResponse);
                    doc1.LoadXml(unwrappedResponse.ToString());
                    //Serialize 
                    Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). Start JSON Serialization. ");
                    jsonText = JsonConvert.SerializeXmlNode(doc1.ChildNodes[0].ChildNodes[0].ChildNodes[0], Newtonsoft.Json.Formatting.None, true);
                    Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). End JSON Serialization. ");
                    ViewBag.Response = responseStr;
                }
                return jsonText;
            }
            ...
            {
                Logger.LogMessage(TracingLevel.INFO, "Method: GetKIISignDetailsOTC(). Error occurred: Inner Exception is " + ex.InnerException.ToString()
                    + "Error Message is " + ex.Message.ToString());
                return null;
            }
        }

        public string TFCNewPolicyPremiumOTC(long ChallanAmount, string Refno, string DDOCODE, string SANORDERNO, string SANDATE, string voucherno, string hoacode, string DEPTCODE, string DEPTPRPSID, string SUBPRPSNAME, string RMTRNAME, string TRSRYCODE)
        {
            string newFile = Server.MapPath("~/MIDocuments/0/" + Refno + ".txt");
            System.IO.FileInfo fi = new System.IO.FileInfo(newFile);
            try
            {
                ...
                using (System.IO.StreamWriter strmwrtr = fi.CreateText())
                {
                    ...
                    strmwrtr.WriteLine("<RctReceiveValidateChlnRq>");
                    strmwrtr.WriteLine("<chlnDate>" + chlntransactiondate + "</chlnDate>");
                    strmwrtr.WriteLine("<deptCode>" + DEPTCODE + "</deptCode>");
                    strmwrtr.WriteLine("<ddoCode>" + DDOCODE + "</ddoCode>");//20139O//12028O
                    strmwrtr.WriteLine("<deptRefNum>" + Refno + "</deptRefNum>");
                    strmwrtr.WriteLine("<rctReceiveValidateChlnDtls>");
                    strmwrtr.WriteLine("<amount>" + ChallanAmount + "</amount>");
                    strmwrtr.WriteLine("<deptPrpsId>" + DEPTPRPSID + "</deptPrpsId>");
                    strmwrtr.WriteLine("<prpsName>" + hoacode + "</prpsName>");//8011~00~105~2~00~000
                    strmwrtr.WriteLine("<subPrpsName>" + SUBPRPSNAME + "</subPrpsName>");
                    strmwrtr.WriteLine("<subDeptRefNum>" + voucherno + "</subDeptRefNum>");
                    strmwrtr.WriteLine("</rctReceiveValidateChlnDtls>");
                    strmwrtr.WriteLine("<rmtrName>" + RMTRNAME + "</rmtrName>");
                    strmwrtr.WriteLine("<totalAmount>" + ChallanAmount + "</totalAmount>");
                    strmwrtr.WriteLine("<trsryCode>" + TRSRYCODE + "</trsryCode>");
                    strmwrtr.WriteLine("</RctReceiveValidateChlnRq>");
                    strmwrtr.WriteLine("</data>");
                }
                return newFile;
        ...
        [Route("mi-dpt-opsoa")]
        public ActionResult MotorInsuranceOTCPaymentStatusOfApplication()
        {
            VM_MotorInsurancePaymentStatus PaymentStatusDetails = null;
            if (Convert.ToInt32(Session["SelectedCategory"]) == 2)
            {
                PaymentStatusDetails = _IMotorInsuranceVehicleDetailsBll.MotorInsuranceotcPaymentStatusBll(Session["UID"].ToString());
            }
            else if (Session["MBVALUE"] != null && Convert.ToBoolean(Session["MBVALUE"]) == true)
            {
                PaymentStatusDetails = _IMotorInsuranceVehicleDetailsBll.MotorInsuranceotcPaymentStatusBll(Session["UID"].ToString());
        ...
        // ICT CHANGE 18-NOV-2021
        public JsonResult InsertMIRenewalChallanDetails(string RefNos, string ChallanAmount, string sanno, DateTime sandate, string voucherno, string hoa, string ddocode, string Type)
        {
            bool isSuccess = false;
            string message = string.Empty;
            string result = string.Empty;
            ...
            if (ChallanAmount == ChalnAmt && AppRefNo == RefNos)
            {
                ...
                if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertRenewalChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
                }
                else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
                {
        ...
        public JsonResult InsertMIRenewalOTCChallanDetails(string RefNos, string ChallanAmount, string sanno, DateTime sandate, string voucherno, string hoa, string ddocode, string Type)
        {
            bool isSuccess = false;
            string message = string.Empty;
            string result = string.Empty;
            ...
            if (ChallanAmount == ChalnAmt && AppRefNo == RefNos)
            {
                ...
                if (Session["SelectedCategory"] != null && Session["SelectedCategory"].ToString().Contains(Convert.ToString((int)UserCategories.DDO)))
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertRenewalotcChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
                }
                else if (Session["Categories"] != null && Session["Categories"].ToString().Contains(Convert.ToString((int)UserCategories.AGENCY)))
                {
                    result = _IMotorInsuranceVehicleDetailsBll.InsertRenewalotcChallanDetailsBll(Convert.ToInt64(Session["UID"]), Convert.ToInt32(Session["SelectedCategory"]), RefNos, Convert.ToInt32(ChallanAmount), sanno, sandate, voucherno, hoa, ddocode, Type);
        ...
        //ICT 27-Nov-2021
        [Route("mi-viewchlrenewotc-det/{ID}")]
        public ActionResult VerifyMIOTCRenewOTCChallanDetails(string ID)
        {
            VM_DDOVerificationDetailsMI ApplicationDetails = null;

            ApplicationDetails = _IMotorInsuranceVehicleDetailsBll.GetotcRenewalChallanDetailsBll(ID, Convert.ToInt64(Session["UID"]));
