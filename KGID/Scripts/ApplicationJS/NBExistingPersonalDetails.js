...
$(document).ready(function () {
    ...
    $('#txtlastmensuration').datetimepicker({
        ...
        minDate: new Date((dob.getFullYear() + 18), dob.getMonth(), dob.getDate()),
        defaultDate: new Date(today.getFullYear(), today.getMonth(), today.getDate()),
        yearStart: new Date((dob.getFullYear() + 18), dob.getMonth(), dob.getDate()),
        yearEnd: new Date(today.getFullYear(), today.getMonth(), today.getDate()),
        scrollMonth: false,
        scrollInput: false,
        closeOnDateSelect: true
    });
...
$(document).ready(function () {
    ...
    if (isMarried == "False") {
        debugger;
        $("#rbtnNo").prop("checked", true);
        $("input[name='ephd_is_pregnant']").attr("disabled", "disabled");
    }
    else {
        $("input[name='ephd_is_pregnant']").removeAttr("disabled");
    }
...
function validatePersonalFields() {
    ...
    if (txtHeight.trim() === '' || (txtHeight.trim() == "0" || txtHeight.trim() == "00" || txtHeight.trim() == "000")) {
        ...
        $("#errHeightReq").text("Please mention valid height(in cms)");
    }
    else if (parseInt(txtHeight.trim()) > 300) {
        $("#errHeightReq").removeAttr("hidden");
        $("#errHeightReq").text("Please mention valid height(in cms)");
    }
    ...
    if (txtWeight.trim() === '' || (txtWeight.trim() == "0" || txtWeight.trim() == "00" || txtWeight.trim() == "000")) {
        ...
        $("#errWeightReq").text("Please mention valid weight(in kgs)");
    }
    else if (parseInt(txtWeight.trim()) > 300) {
        $("#errWeightReq").removeAttr("hidden");
        $("#errWeightReq").text("Please mention valid weight(in kgs)");
    }
    ...
    $(".txtar-personal-req").each(function () {
        if ($(this).val() === '') {
            ...
            $(this).next("label.err").text("Please enter details");
        }
    });
    debugger;
    if ($("input[name='IsInfectiousDiseaseHouse']:checked").val() == null) {
        $("#errtxtIsInfectiousDiseaseHouseReq").removeAttr("hidden");
        $("#errtxtIsInfectiousDiseaseHouseReq").text("Please select radio button (Yes/No)");
    }
    if ($("input[name='IsAbsent']:checked").val() == null) {
        $("#errtxtIsAbsentReq").removeAttr("hidden");
        $("#errtxtIsAbsentReq").text("Please select radio button (Yes/No)");
    }
    if ($("input[name='IsStomachDisease']:checked").val() == null) {
        $("#errtxtIsStomachDiseaseReq").removeAttr("hidden");
        $("#errtxtIsStomachDiseaseReq").text("Please select radio button (Yes/No)");
    }
    if ($("input[name='IsDrinksDrugs']:checked").val() == null) {
        $("#errtxtIsDrinksDrugsReq").removeAttr("hidden");
        $("#errtxtIsDrinksDrugsReq").text("Please select radio button (Yes/No)");
    }
    if ($("input[name='IsAnyOtherDisease']:checked").val() == null) {
        $("#errtxtIsAnyOtherDiseaseReq").removeAttr("hidden");
        $("#errtxtIsAnyOtherDiseaseReq").text("Please select radio button (Yes/No)");
    }
...
$(".display-as-label-upload").hover(function () {
    $(this).attr("title", "Upload signed original documents");
})
