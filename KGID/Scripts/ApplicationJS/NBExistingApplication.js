                ...
                if (currentIndex == 0) {
                    ...
                    if (isMarried !== "True" && isOrphan === "True")
                        ...
                        if ($("#tblFamilyDetails TBODY TR").find("td").hasClass("dataTables_empty")) {
                            ...
                            if ((isMarried == "False" || isMarried == "True") && (isOrphan == "False" || (isOrphan == undefined && $('#txtFatherName').val() != "")) && isParentsAdded < 2) {
...
function SaveBasicDetails(eid) {
    ...
    var hd = $("#hdnOrphan").val();
    var spousename = $("#txtSpouseName").val();
    var spousenameinkannda = $("#txtSpouseNameKN").val();
    var txtCSKN = $("#txtCSKN").val();
    ...
    var pincode = $("#txtEPin").val();
    var Currentsp = $("#txtCS").val();
    var CurrentSpink = $("#txtCSKN").val();
    ...
    //if (grp == "") { $("#errEmpGrp").removeAttr('hidden'); }
    var formData = new FormData($("#frmAppBasicDetails").get(0));
    formData.append("spouse_name", spousename);
    formData.append("spouse_name_kannada", spousenameinkannda);
    formData.append("Current_spouse_name_kannada", CurrentSpink);
    $.ajax({
...
function GetKGIDDetails() {
    GetBasicDetails();
    $.ajax({
        url: '/Employee/ExistingKGIDDetailsToView',
        data: JSON.stringify({}),
        type: 'POST',
        async: false,
        contentType: 'application/json; charset=utf-8',
...
function GetFamilyDetails() {
    ...
    $.ajax({
        url: '/Employee/ExistingFamilyDetailsToView',
        data: JSON.stringify({}),
        type: 'POST',
        async: false,
        contentType: 'application/json; charset=utf-8',
...
function GetPersonalDetails() {
    $.ajax({
        url: '/Employee/ExistingPersonalDetailsToView',
        data: JSON.stringify({}),
        type: 'POST',
        async: false,
        contentType: 'application/json; charset=utf-8',
...
function GetDeclaration() {
    $.ajax({
        url: '/Employee/ExistingDeclarationDetailsToView',
        data: JSON.stringify({}),
        type: 'POST',
        async: false,
        contentType: 'application/json; charset=utf-8',
