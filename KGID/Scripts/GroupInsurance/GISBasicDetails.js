...
function ValidateBasicDetailsForGis()
{
    debugger;
    $('.err').attr("hidden", true);
    var result = true;
    if ($("#txtEmployeeFullNameKN").length > 0 && ($("#txtEmployeeFullNameKN").val()).trim() != "")  { }
    else { $("#errProposerNameKannada").removeAttr("hidden"); result = false; }

    if ($("#txtFatherNameKN").length > 0 && ($("#txtFatherNameKN").val()).trim() != "") {  }
    else { $("#errFatherNameKannada").removeAttr("hidden"); result = false; }

    if  ($("#txtEAddress").length > 0 && ($("#txtEAddress").val()).trim() != "") { }
    else { $("#errEAddress").removeAttr("hidden"); result = false;  }

    if ($("#txtEPin").length > 0 && ($("#txtEPin").val()).trim() != "" && ($("#txtEPin").val()).trim() != 0)  { }
    else { $("#errEPin").removeAttr("hidden"); result = false; }


    var first_char = $("#txtEPin").val().substring(0, 1);
    if (first_char != 0) { }
    else { $("#errEPin").removeAttr("hidden"); result = false; }
...
function validatePincode()
{
    var res = false;
    var a = /(^\d{6}$)/;
    if (a.test($("#txtEPin").val())) {
       res= true;
    }
    else
    {
        res = false;
        $("#errEPin").removeAttr("hidden");
        /*$("#errEPinValidate").removeAttr("hidden");*/
       
    }
    return res;
}

$(txtEmployeeFullNameKN).keyup(function (event) {

    $("#errProposerNameKannada").attr("hidden", true);;
});

$(txtFatherNameKN).keyup(function (event) {
...
$(txtEPin).keyup(function (event) {
    ...
    $("#errEPin").attr("hidden", true);;
    $("#errEPinValidate").attr("hidden", true);;
});

$(txtEAddress).keyup(function (event) {

    $("#errEAddress").attr("hidden", true);;
});

function SaveBasicDetails123() {
    debugger;
    $("#hdnmarriedstatus").val($("#ddlDRType").val());
    $("#lblEmployeeName").html($("#txtEmployeeFullName").val())
    $("#txtEmployeeFullName").val()
    //var result = isKannadaWord($("#txtEmployeeFullNameKN").val())
