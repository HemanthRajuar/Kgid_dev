...
$(".modal-save").click(function () {
    ...
    if (ValidateEmpDetails(viewModel)) {
        ...
        if ($('.err:visible').length === 0) {
            ...
            var proposername = $("#txtName").val();
            var PlaceOfbirth = $("#txtPlaceOfBirth").val();
            var Email = $("#txtEmailId").val();
            var pan = $("#txtPANNumber").val();
            var payscale = $("#ddlewd_payscle_code").val();
            var design = $("#ddlewd_designation").val();
            var grp = $("#ddlewd_group").val();
            var Gender = $("#ddlGender").val();
            if (Gender == "") { $("#errGender").removeAttr('hidden'); }
            if (mob == "") { $("#errMblNum").removeAttr('hidden'); }
            if (proposername == "") { $("#errEmpName").removeAttr('hidden'); }
            if (PlaceOfbirth == "") { alert(PlaceOfbirth); $("#errPOB").removeAttr('hidden'); }
            if (Email == "") { $("#errMailID").removeAttr('hidden'); }
            if (pan == "") { $("#errPANNum").removeAttr('hidden'); }
            if (payscale == "") { $("#errPSCode").removeAttr('hidden'); }
            if (design == "") { $("#errDesg").removeAttr('hidden'); }
