    ...
    public class NBApplicationDll : INBApplicationDll
    {
        ...
        public int SaveEmployeeBasicDetails(VM_EditEmployeeDetails employeeDetails)
        {
            ...
            if (newEmpDetails != null)
            {
                ...
                newEmpDetails.nebd_active = true;
            }
        ...
        public long RestrictNomineeDetails(long EmpId)
        {
            ...
            try
            {
                SqlParameter[] sqlparam =
               {
                    new SqlParameter("@end_emp_id",EmpId),
                };
                result = Convert.ToInt64(_Conn.ExecuteCmd(sqlparam, "sp_kgid_restrictInsertNomineedetails"));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        //    return result;
        //}
        public string SaveNBBasic(VM_BasicDetails employeeBasicDetails)
        {
            int panCount = 0; int emailCount = 0; var DuplicateDetails = "";
            try
            {
                panCount = Convert.ToInt16(employeeBasicDetails.mailPanCount);
                emailCount = Convert.ToInt16(employeeBasicDetails.mailEmailCount);
                if (panCount == 1 && emailCount == 1)
                {
                    var CheckDupEmail1 = (from emp in _db.tbl_employee_basic_details
                                          where ((emp.email_id == employeeBasicDetails.email_id) && emp.employee_id != employeeBasicDetails.employee_id)
                                          select emp).Count();
                    var CheckDupPanNo1 = (from emp in _db.tbl_employee_basic_details
                                          where ((emp.pan_number == employeeBasicDetails.pan_number) && emp.employee_id != employeeBasicDetails.employee_id)
                                          select emp).Count();
                    if (CheckDupEmail1 > 0)
                    {
                ...
                if (DuplicateDetails == "")
                {
                    ...
                    {
                    //new SqlParameter("@Referance_number",RefNo),
                    new SqlParameter("@employee_name_kannada",employeeBasicDetails.employee_name_kannada),
                    new SqlParameter("@father_name",employeeBasicDetails.father_name),
                    new SqlParameter("@father_name_kannada",employeeBasicDetails.father_name_kannada),
                    new SqlParameter("@place_of_birth",employeeBasicDetails.place_of_birth),
                    new SqlParameter("@email_id",employeeBasicDetails.email_id),
                    new SqlParameter("@pan_number",employeeBasicDetails.pan_number),
                    ...
                    new SqlParameter("@dr_id",employeeBasicDetails.dr_id),
                    new SqlParameter("@Current_spouse_name",employeeBasicDetails.Current_spouse_name),
                    new SqlParameter("@Current_spouse_name_kannada",employeeBasicDetails.Current_spouse_name_kannada),
                    new SqlParameter("@divorce_upload_path",employeeBasicDetails.divorced_upload_doc_path)
                };
                    DuplicateDetails = (_Conn.ExecuteCmd(sqlparam, "sp_kgid_insertBasicdetails"));
                }
            }
            catch (Exception ex)
            {
        ...
        public VM_BasicDetails NewEmployeeBasicDetailsDll(long EmployeeCode)
        {
            ...
            try
            {
                ...
                if (dsBD.Tables[0].Rows.Count > 0)
                {
                    ...
                    objBD.Current_spouse_name_kannada = dsBD.Tables[0].Rows[0]["Current_spouse_name_kannada"].ToString();
        ...
        public VM_PolicyDetails KGIDDetailsDll(long EmployeeCode)
        {
            ...
            try
            {
                ...
                //}
                var employee = _db.tbl_employee_work_details.Where(a => a.ewd_emp_id == EmployeeCode).FirstOrDefault();
                var employeework = _db.tbl_employee_basic_details.Where(a => a.employee_id == EmployeeCode).FirstOrDefault();

                objPD.payscale_id=employee.ewd_payscale_id;
                if(employeework.first_kgid_policy_no != null && (dsPD.Tables[1].Rows.Count > 0 || dsPD.Tables[0].Rows.Count > 0)) {
                    objPD.p_premium = Convert.ToDouble(objPD.premium_Amount); }
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<tbl_verification_details> ExistingKGIDDetailsDll(long? policyno)
        {
            var VerificationDetails = new List<tbl_verification_details>();
            var company = _db.tbl_verification_details.Where(c => c.FIRST_KGIDNO_HRMS == policyno).SingleOrDefault();
           // VerificationDetails = (from emplo in _db.tbl_verification_details.AsNoTracking().Where(a =>  a.FIRST_KGIDNO_HRMS.Equals(null) ==  policyno)  select emplo).ToList();
            var data = (from a in _db.tbl_employee_basic_details
                        join b in _db.tbl_verification_details on a.first_kgid_policy_no equals  b.FIRST_KGIDNO_HRMS

                        select new tbl_verification_details
                        { 
                           //FIRST_KGIDNO_HRMS= string.IsNullOrEmpty(a.first_kgid_policy_no) ? 0 : int.Parse(a.first_kgid_policy_no),
                         FATHER_NAME_HRMS = a.father_name 
        ...
        public void SaveOtherData(long emd_emp_id, long emd_application_id, int emd_medical_health_id, bool? emd_status, string emd_remarks, HttpPostedFileBase medicalDoc, string emd_upload_document_path, string medDocType)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(emd_remarks))
                    drMD["emd_status"] = false;
                else
                ...
                if (dsND.Tables.Count > 0)
                {
                    if (dsND.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in dsND.Tables[0].Rows)
                        {
                            ...
                            if (nomineeDetail.Relation == "Other")
        ...
        public VM_PersonalHealthDetails PersonalDetailsDll(long EmployeeCode)
        {
            ...
            //objPD.IsInfectiousDiseaseHouse = false;
                ...
                if (dsPD.Tables.Count > 0)
                {
                    ...
                    if (dsPD.Tables.Count == 2)
                    {
                        if (dsPD.Tables[1].Rows.Count > 0)
                        {
                            ...
                            {
                                for (int i = 0; i < dsPD.Tables[1].Rows.Count; i++)
                                {
                                    ...
                                    if (!PersonalData)
                                    {
                                        ...
                                        if (dsPD.Tables[1].Rows[0]["ephd_health_condition"] != DBNull.Value)
                                            objPD.ephd_health_condition = Convert.ToBoolean(dsPD.Tables[1].Rows[0]["ephd_health_condition"]);
                                        if (dsPD.Tables[1].Rows[0]["ephd_is_pregnant"] != DBNull.Value)
                                            objPD.ephd_is_pregnant = Convert.ToBoolean(dsPD.Tables[1].Rows[0]["ephd_is_pregnant"]);
                                        ...
                                        objPD.IsInfectiousDiseaseHouse = false;
                                        objPD.IsStomachDisease = false;
                                        objPD.IsAnyOtherDisease = false;
                                        objPD.IsDrinksDrugs = false;
                                        objPD.IsAbsent = false;
                                    ...
                                    if (Convert.ToInt16(dsPD.Tables[1].Rows[i]["epdd_personal_health_id"]) == 3)
                                    {
                                        objPD.IsInfectiousDiseaseHouse = true;
                                        objPD.InfectiousDiseaseHouseDetails = dsPD.Tables[1].Rows[i]["epdd_remarks"].ToString();
                                        //objPersonal.InfectiousDiseaseHouseDocFileName = GetFileName(PersonalDisDetails[i].pdd_disease_doc);
                                        objPD.InfectiousDiseaseHouseDocFileName = dsPD.Tables[1].Rows[i]["epdd_upload_document_path"].ToString().Replace(@"\", @"/");
                                    ...
                                    //}
                                    else if (Convert.ToInt16(dsPD.Tables[1].Rows[i]["epdd_personal_health_id"]) == 7)
                                    {
                                        objPD.IsStomachDisease = true;
                                        objPD.StomachDiseaseDetails = dsPD.Tables[1].Rows[i]["epdd_remarks"].ToString();
                                        objPD.StomachDiseaseDocFileName = dsPD.Tables[1].Rows[i]["epdd_upload_document_path"].ToString().Replace(@"\", @"/");
                                    }
                                    ...
                                    //}
                                    else if (Convert.ToInt16(dsPD.Tables[1].Rows[i]["epdd_personal_health_id"]) == 10)
                                    {
                                        objPD.IsAnyOtherDisease = true;
                                        objPD.AnyOtherDiseaseDetails = dsPD.Tables[1].Rows[i]["epdd_remarks"].ToString();
                                        objPD.AnyOtherDiseaseDocFileName = dsPD.Tables[1].Rows[i]["epdd_upload_document_path"].ToString().Replace(@"\", @"/");
                                    }

                                    else if (Convert.ToInt16(dsPD.Tables[1].Rows[i]["epdd_personal_health_id"]) == 11)
                                    {
                                        objPD.IsDrinksDrugs = true;
                                        objPD.DrinksDrugsDetails = dsPD.Tables[1].Rows[i]["epdd_remarks"].ToString();
                                        objPD.DrinksDrugsDocFileName = dsPD.Tables[1].Rows[i]["epdd_upload_document_path"].ToString().Replace(@"\", @"/");
                                    }

                                    else if (Convert.ToInt16(dsPD.Tables[1].Rows[i]["epdd_personal_health_id"]) == 12)
                                    {
                                        objPD.IsAbsent = true;
                                        objPD.AbsentDetails = dsPD.Tables[1].Rows[i]["epdd_remarks"].ToString();
                                        objPD.AbsentDocFileName = dsPD.Tables[1].Rows[i]["epdd_upload_document_path"].ToString().Replace(@"\", @"/");
                                    }
            ...
            try
            {
                ...
                if (CheckDupEmpDetails < 1)
                {
                    if (newEmpDetails != null)
                    {
                        ...
                        newEmpDetails.active_status = true;
        ...
        public VM_DeptVerificationDetails GetUploadedDocuments(long empId, long applicationId)
        {
            VM_DeptVerificationDetails verificationDetails = new VM_DeptVerificationDetails();
        ...
        public string GetUserDetailsbasedOnMobNum(long MobileNumber)
        {
            ...
            try
            {
                ...
                result = _Conn.ExecuteCmd(sqlparam, "sp_kgid_getUserDetailsbasedOnMobNum");
        ...
        //public string GetpaymentapplicationDetails(string KGIDNumber)
        //{
        //    string result = "0";
        //    //try
        //    {

        //        return (from appno in _db.tbl_kgid_application_details.Where(a => a.kad_kgid_application_number == KGIDNumber)
        //                join ews in _db.tbl_employee_basic_details.AsNoTracking().Where(p => p.active_status == true) on appno.kad_emp_id equals ews.employee_id into employeename
        //                from empdetails in employeename.DefaultIfEmpty()
        //                join challensdet in _db.tbl_challan_details.AsNoTracking().Where(p => p.cd_status == 1) on appno.kad_application_id equals challensdet.cd_application_id into challen
        //                from challendetails in challen.DefaultIfEmpty()
        //                join challenstatus in _db.tbl_challan_status.AsNoTracking().Where(p => p.cs_active_status == 1) on challendetails.cd_id equals challenstatus.cs_challan_id //challensdet.cd_id equals challenstatus.cs_challan_id
        //                select new VM_ApplicationDetail
        //                {
        //                    Employe_name = empdetails.employee_name,
        //                    Emp_Id = empdetails.employee_id,
        //                    ApplicationNumber = appno.kad_kgid_application_number,
        //                    ApplicationId = appno.kad_application_id,
        //                    Challen_TransactionNo = challenstatus.cs_transaction_ref_no,
        //                    PaymentStatus = challenstatus.cs_status,
        //                    amount = challenstatus.cs_amount,
        ...
        public List<VM_ApplicationDetail> GetpaymentapplicationDetails(string KGIDNumber)
        {
            ...
            {
                return (from appno in _db.tbl_kgid_application_details.Where(a => a.kad_kgid_application_number == KGIDNumber)
                        join cd in _db.tbl_challan_details.AsNoTracking().Where(p => p.cd_active_status == 1 || p.cd_active_status == 0) on appno.kad_application_id equals cd.cd_application_id into cd1
                        from cdd in cd1.DefaultIfEmpty()
                        join cs in _db.tbl_challan_status.AsNoTracking().Where(p => p.cs_active_status == 1) on cdd.cd_challan_id equals cs.cs_challan_id into cs1
                        from css in cs1.DefaultIfEmpty()
                        join ews in _db.tbl_employee_basic_details.AsNoTracking().Where(p => p.active_status == true) on appno.kad_emp_id equals ews.employee_id into emp1
                        from ewss in emp1.DefaultIfEmpty()
                            /* join csm in _db.tbl_challan_status_master.AsNoTracking().Where(p => p.csm_active_status == 1) on css.cs_status equals csm.csm_status_id into csm1
                             from csms in csm1.DefaultIfEmpty()*/
                        select new VM_ApplicationDetail
                        {

                            Employe_name = ewss.employee_name,
                            Emp_Id = ewss.employee_id,
                            ApplicationNumber = appno.kad_kgid_application_number,
                            ApplicationId = appno.kad_application_id,
                            Challen_TransactionNo = css.cs_transaction_ref_no,
                            PaymentStatus = css.cs_status != null ? (int)css.cs_status : 0,
                            challan_status_id = css.cs_challan_status_id != null ? css.cs_challan_status_id : 0,
                            amount = cdd.cd_amount,
                            cd_active_status = cdd.cd_active_status,
                            cd_challan_ref_no = cdd.cd_challan_ref_no,
                            cd_challan_id = cdd.cd_challan_id,
                        }).ToList();
            }
            catch (Exception ex)
            {
                Logger.LogMessage(TracingLevel.INFO, "payment Verification Details - Admin" + ex.Message);
                return null;
            }
        }
        //Delete from tbl_challan_details
        ...
        {
            string result = "0";
            int count = int.Parse(ChallanStatusRowCount);
            try
            {
                SqlParameter[] sqlparam =
                {
                   new SqlParameter("@ChallanId",cd_challan_id),
                    new SqlParameter("@ChallanRefNo",cd_challan_ref_no),
                };
                result = _Conn.ExecuteCmd(sqlparam, "sp_kgid_DeleteChallanDetails");

            }
            catch (Exception ex)
            {
                Logger.LogMessage(TracingLevel.INFO, "DeleteChallanDetails-ChallanDetails - Admin" + ex.Message);
                result = "0";
            }
...

            {
                try
                {
                   ...
                   new SqlParameter("@ApplicationId",applicationId),
                    ...
                    result = _Conn.ExecuteCmd(sqlparam, "sp_kgid_UpdateChallanDetails");

                }
                catch (Exception ex)
                {
                    Logger.LogMessage(TracingLevel.INFO, "UpdateChallanDetails-ChallanDetails - Admin" + ex.Message);
                    result = "0";
                }
            }
            return result;
        ...
        public string UpdatePaymentVerification(long paymentstatus, long challanId)
        {
            ...
            try
            {
                ...
                result = _Conn.ExecuteCmd(sqlparam, "sp_kgid_updateChallanStatus");

            }
            catch (Exception ex)
            {
                Logger.LogMessage(TracingLevel.INFO, "UpdatePaymentDetails-Challanstatus - Admin" + ex.Message);
                result = "0";
            }
            return result;
        }

        public string UpdateUserDetails(long KGIDNumber, long MobileNo, string EmailID, long EmployeeId)
            ...
            try
            {
                ...
                if (dsPD.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow dr in dsPD.Tables[0].Rows)
                    {
                        ...
                        objDetails.RM_Remarks_Desc = (dr["RM_Remarks_Desc"]).ToString();
                        objDetails.RM_Comments = (dr["RM_Comments"]).ToString();
                        objDetails.active_status = (dr["kawt_application_status"]).ToString();
                        objDetails.RM_Remarks_Desc = (dr["RM_Remarks_Desc"]).ToString();
                        objDetails.kawt_application_id = (dr["kawt_application_id"]).ToString();
                        objDetails.employee_id = (dr["employee_id"]).ToString();
                        obj.Add(objDetails);
                    }
        ...
        //
        public async Task<bool> UpdateUserDetails1(long KGIDNumber, long MobileNo, string EmailID, long EmployeeId)
        {
            using (DbConnectionKGID context = new DbConnectionKGID())
            {
                tbl_employee_basic_details ebd = await context.tbl_employee_basic_details.Where(a => a.employee_id == EmployeeId).FirstOrDefaultAsync();
                if (ebd != null)
                {
                    ebd.mobile_number = MobileNo;
                    ebd.email_id = EmailID;
        ...
        public async Task<bool> IsMobNoAlreadyExistsAsync(long KGIDNumber, long MobileNumber, long EmployeeId, string EMailID)
        {
            using (DbConnectionKGID context = new DbConnectionKGID())
            {
                var query = (context.tbl_employee_basic_details.AsNoTracking().
                    Where(m => (
                        m.mobile_number == MobileNumber) ||
                        m.email_id == EMailID));

                query = query.Where(a => a.employee_id != EmployeeId);
        ...
        public async Task<bool> UpdatePaymentVerification1(long paymentstatus, long challanId)
        {
            using (DbConnectionKGID context = new DbConnectionKGID())
            {
                tbl_challan_status cs = await context.tbl_challan_status.Where(a => a.cs_challan_id == challanId).FirstOrDefaultAsync();
                if (cs != null)
                {
                    cs.cs_status = paymentstatus;
                    await context.SaveChangesAsync();
                    return true;
                }
                return false;
        ...
        public async Task<bool> DeleteForPaymentVerification1(long cd_challan_id, string cd_challan_ref_no, long applicationId)
        {
            bool result = false;
            using (DbConnectionKGID context = new DbConnectionKGID())
            {
                try
                {
                    context.Database.ExecuteSqlCommand("Delete tbl_challan_details where cd_challan_id = {0}", new object[] { cd_challan_id });
                    result = true;
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    tbl_challan_details cd = await context.tbl_challan_details.Where(a => a.cd_application_id == applicationId).FirstOrDefaultAsync();
                    if (cd != null)
                    {
                        cd.cd_active_status = 1;
                        await context.SaveChangesAsync();
                        result = true;
                    }
                }
        ...
        public List<VM_ApplicationDetail> GetApplicationMappingDetails(string ApplicationRefnum)
        {
            try
            {
                return (from appno in _db.tbl_kgid_application_details.Where(a => a.kad_kgid_application_number == ApplicationRefnum && a.kad_active_status == true)
                        join kawf in _db.tbl_kgid_application_workflow_details.Where(a => a.kawt_active_status == true && (a.kawt_application_status != 15 || a.kawt_application_status != 3)) on appno.kad_application_id equals kawf.kawt_application_id into kawf1
                        from kawfs in kawf1.DefaultIfEmpty()
                        join ews in _db.tbl_employee_basic_details.AsNoTracking().Where(p => p.active_status == true) on appno.kad_emp_id equals ews.employee_id into emp1
                        from ewss in emp1.DefaultIfEmpty()
                        join assignedTo in _db.tbl_employee_basic_details.AsNoTracking().Where(p => p.active_status == true) on kawfs.kawt_assigned_to equals assignedTo.employee_id into assignedTo1
                        from assignedTos in assignedTo1.DefaultIfEmpty()
                        join verifiedBy in _db.tbl_employee_basic_details.AsNoTracking().Where(p => p.active_status == true) on kawfs.kawt_verified_by equals verifiedBy.employee_id into verifiedBy1
                        from verifiedBys in verifiedBy1.DefaultIfEmpty()
                        join asm in _db.tbl_application_status_master.AsNoTracking().Where(p => p.asm_active == 1) on kawfs.kawt_application_status equals asm.asm_status_id into asm1
                        from asms in asm1.DefaultIfEmpty()
                        join rm in _db.tbl_remarks_master.AsNoTracking().Where(p => p.RM_Active_Status == true) on kawfs.kawt_remarks equals rm.RM_Remarks_id.ToString() into rm1
                        from rms in rm1.DefaultIfEmpty()
                        ...
                        select new VM_ApplicationDetail
                        {
                            ...
                            ApplicationNumber = appno.kad_kgid_application_number,
                            ApplicationId = appno.kad_application_id,
                            kawt_assigned_to = kawfs.kawt_assigned_to != null ? (long)kawfs.kawt_assigned_to : 0,
                            kawt_verified_by = kawfs.kawt_verified_by != null ? (long)kawfs.kawt_verified_by : 0,
                            kawt_remarks = kawfs.kawt_remarks,
                            kawt_comments = kawfs.kawt_comments,
                            kawt_workflow_id = kawfs.kawt_workflow_id,
                            kawt_application_status = kawfs.kawt_application_status,
                            kawt_remarks_discription = rms.RM_Remarks_Desc,
                            kawt_AssignedTo_Name = assignedTos.employee_name,
                            kawt_verifiedBy_Name = verifiedBys.employee_name,
                            kawt_applStatus_discription = asms.asm_status_desc,
                            kawt_AssignedTo_KgidNo = assignedTos.first_kgid_policy_no != null ? (long)assignedTos.first_kgid_policy_no : 0,
                            kawt_verifiedBy_KgidNo = verifiedBys.first_kgid_policy_no != null ? (long)verifiedBys.first_kgid_policy_no : 0,

                        }).ToList();
            }
            catch (Exception ex)
            {
                Logger.LogMessage(TracingLevel.INFO, "Update Apliccation mapping(Display) - Admin" + ex.Message);
                return null;
            }
        }

        public List<VM_ApplicationDetail> GetemployeeDetailsBasedOfKGIDNo(long KGIDno)
        {
            try
            {
                return (from ewss in _db.tbl_employee_basic_details.Where(a => a.first_kgid_policy_no == KGIDno && a.active_status == true)
                        join rm in _db.tbl_role_mapping.Where(a => a.rm_isactive == 1) on ewss.employee_id equals rm.rm_emp_id
                        select new VM_ApplicationDetail
                        {
                            Employe_name = ewss.employee_name,
                            Emp_Id = ewss.employee_id,
                            first_kgid_policy_no = (long)ewss.first_kgid_policy_no != null ? (long)ewss.first_kgid_policy_no : 0,
                            roleid = rm.rm_category_id

                        }).ToList();
            }
            catch (Exception ex)
            {
                Logger.LogMessage(TracingLevel.INFO, "Display Employee Details - Admin" + ex.Message);
                return null;
            }
        }

        public async Task<bool> UpdateAssignedTo(long EmployeeId, long kawt_workflow_id, int kawt_application_status)
        {
            using (DbConnectionKGID context = new DbConnectionKGID())
            {
                KGID_Models.KGID_Verification.tbl_kgid_application_workflow_details awd = await context.tbl_kgid_application_workflow_details.Where(a => a.kawt_workflow_id == kawt_workflow_id).FirstOrDefaultAsync();
                if (awd != null)
                {
                    awd.kawt_assigned_to = EmployeeId;
                    if (kawt_application_status != 0)
                    { awd.kawt_application_status = kawt_application_status; }
                    await context.SaveChangesAsync();
                    return true;
                }
                return false;
