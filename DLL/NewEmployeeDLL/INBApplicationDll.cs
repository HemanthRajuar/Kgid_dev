...
namespace DLL.NewEmployeeDLL
{
    public interface INBApplicationDll
    {
        ...
        IEnumerable<tbl_verification_details> ExistingKGIDDetailsDll(long? policyno);
        //New
        string SaveNBBasic(VM_BasicDetails employeeBasicDetails);
        ...
        List<VM_ApplicationDetail> GetApplicationMappingDetails(string ApplicationRefnum);
        List<VM_ApplicationDetail> GetemployeeDetailsBasedOfKGIDNo(long KGIDno);
        Task<bool> UpdateAssignedTo(long EmployeeId, long kawt_workflow_id, int kawt_application_status);
    }
