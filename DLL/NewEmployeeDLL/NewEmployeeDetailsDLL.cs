    ...
    public class NewEmployeeDetailsDLL : INewEmployeeDetailsDLL
    {
        ...
        public VM_InsuredEmployeeLoginDetail VerifyKGIDAndMobileNo(string kgId, long mobileNumber)
        {
            ...
            if (dsBD.Tables[0].Rows.Count > 0)
            {
                ...
                employeeLoginDetail.EmployeeName = dsBD.Tables[0].Rows[0]["employee_name"].ToString(); 
                employeeLoginDetail.UserCategory = dsBD.Tables[0].Rows[0]["user_category_id"].ToString();
                employeeLoginDetail.Moduletype = dsBD.Tables[0].Rows[0]["Moduletype"].ToString();
        ...
        public VM_BasicDetails GetEmployeeData(string kgidnum)
        {
            ...
            if (dsBD.Tables[0].Rows.Count > 0)
            {
                ...
                objDetl.Moduletype = dsBD.Tables[0].Rows[0]["Moduletype"].ToString();
            }
            return objDetl;
        }
        ...
        public void SaveCategoryDetailsData(int? DistID, int CategoryID, int DDOCodeID, long EmpId, bool status, int ModuleType)
        {
            try
            {
                ...
                tbl_assign_application UserDtls = new tbl_assign_application();
                 UserDtls = (from ap in _db.tbl_assign_application.Where(n => n.ap_emp_id == EmpId && n.ap_category_id == CategoryID)
                                select ap).FirstOrDefault();
                if(UserDtls != null)
                {
                    ////tbl_assign_application details = new tbl_assign_application();
                    ////details = _db.tbl_assign_application.Where(n => n.ap_emp_id == EmpId && n.ap_category_id == CategoryID && ModuleType!=4).FirstOrDefault();
                    ////details.ap_category_id = CategoryID;
                    ////details.ap_emp_id = EmpId;
                    ////details.ModuleType = 1;                    
                    ////details.ap_created_by = 1;
                    //UserDtls.ap_creation_datetime = DateTime.UtcNow;
                    ////details.ap_updated_by = 1;
                    //UserDtls.ap_updation_datetime = DateTime.UtcNow;
                    //UserDtls.ap_active_status = status;
                    ////_db.Entry(details).State = EntityState.Modified;                   
                    //_db.SaveChanges();
                    SqlParameter[] sqlparam1 =
                        {
                        new SqlParameter("@eid",1),
                        new SqlParameter("@ap_dist_id",DistID),
                        new SqlParameter("@ap_category_id",CategoryID),
                        new SqlParameter("@ap_ddocode_id",DDOCodeID),
                        new SqlParameter("@ap_active_status",status),
                        new SqlParameter("@ModuleType",ModuleType),
                        new SqlParameter("@EmpID",EmpId),                        
                        new SqlParameter("@UAP",1),                        
                    };
                    if(UserDtls.ModuleType != ModuleType)
                    {
                        SqlParameter[] sqlparam11 =
                       {
                        new SqlParameter("@eid",1),
                        new SqlParameter("@ap_dist_id",DistID),
                        new SqlParameter("@ap_category_id",CategoryID),
                        new SqlParameter("@ap_ddocode_id",DDOCodeID),
                        new SqlParameter("@ap_active_status",status),
                        new SqlParameter("@ModuleType",UserDtls.ModuleType),
                        new SqlParameter("@EmpID",EmpId),
                        new SqlParameter("@UAP",3),
                    };

                        var resulttemp = Convert.ToInt32(_Conn.ExecuteCmd(sqlparam11, "sp_kgid_AssignApplicationRole"));
                    }
                    var result1 = Convert.ToInt32(_Conn.ExecuteCmd(sqlparam1, "sp_kgid_AssignApplicationRole"));
                    UserDtls = null;
                }
                else
                {
                    //tbl_assign_application userdetails = new tbl_assign_application();
                    //userdetails.ap_category_id= CategoryID;
                    //userdetails.ap_emp_id = EmpId;
                    //userdetails.ModuleType = 1;
                    //userdetails.ap_no_of_application = default(long);
                    //userdetails.ap_created_by = default(long);
                    ////userdetails.ap_creation_datetime = DateTime.UtcNow;
                    //userdetails.ap_updated_by = default(long);
                    ////userdetails.ap_updation_datetime = DateTime.UtcNow;
                    //userdetails.ap_active_status = status;
                    //userdetails.ap_dist_id = Convert.ToInt32(DistID);
                    //userdetails.ap_ddocode_id = DDOCodeID;
                    ////userdetails.ap_assign_date = DateTime.UtcNow;
                    //_db.Entry(userdetails).State = EntityState.Added;
                    //_db.tbl_assign_application.Add(userdetails);
                    //_db.SaveChanges();
                    if (ModuleType == 1)
                    {
                        SqlParameter[] sqlparam1 =
        ...
        public int SaveCategoryDetailsDll (long kgidmun, int ddo,int cw, int avgcw, int sup,int dio, int dd, int d, int AD, int nb, int loan, int claims, int motor, int odclaims,int mvcclaims, long empId)
        {
            ...
            if (temp)
            {
                ...
                if (cw == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 3, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 3, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (avgcw == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 10, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 10, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (sup == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 4, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 4, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (dio == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 5, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 5, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (AD == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 15, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 15, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (dd == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 6, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 6, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                if (d == 1)
                {
                    SaveCategoryDetailsData(Res.DistictID, 7, Res.DDOID, Res.EmpID, true, ModuleType);
                }
                else
                {
                    SaveCategoryDetailsData(Res.DistictID, 7, Res.DDOID, Res.EmpID, false, ModuleType);
                }
                ...
                try
                {
                    ...
                    new SqlParameter("@Module",modules)
                };
